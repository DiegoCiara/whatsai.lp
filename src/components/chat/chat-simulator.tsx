...
const ChatSimulator = ({ mobile = false }: ChatSimulatorProps) => {
  ...
  const messages = [
    { id: 1, position: 'right', content: 'Olá, preciso de ajuda com meu pedido, pode me ajudar?', time: '10:30', read: false },
    { id: 2, position: 'left', content: 'Claro! Qual é o problema com o seu pedido?', time: '10:31', read: true },
    { id: 3, position: 'right', content: 'Meu pedido não chegou e já faz uma semana.', time: '10:32', read: false },
    { id: 4, position: 'left', content: 'Sinto muito por isso! Vou verificar o status do seu pedido agora.', time: '10:33', read: true },
    { id: 5, position: 'right', content: 'Obrigado pela ajuda! 😊', time: '10:34', read: false }
  ];
  ...
  return (
    ...
    >
      <div className="header-color p-3 flex items-center h-14 md:h-16">
        <div className={`${mobile ? 'w-8 h-8' : 'w-10 h-10'} rounded-full bg-white flex items-center justify-center mr-3`}>
      ...
      <div className="bg-white p-2 md:p-3 flex items-center min-h-14">
        ...
        <div className="flex-1 bg-gray-100 rounded-full py-2 px-3 md:px-4 mx-1 md:mx-2">
          <div className="text-gray-500 text-sm md:text-base">Digite uma mensagem</div>
        </div>
  ...
  return (
    ...
    >
      <div className={`${position === 'left' ? 'message-bubble rounded-lg rounded-tl-none' : 'bg-green-100 rounded-lg rounded-tr-none'} py-1 md:py-2 px-2 md:px-3 inline-block shadow-sm`}>
        <div className={`text-gray-800 ${mobile ? 'text-xs' : 'text-sm'}`}>{content}</div>
        <div className={`${mobile ? 'text-xxs' : 'text-xs'} text-gray-500 text-right mt-1`}>
          {time}
          {read && position === 'right' && (
            <>
              <svg xmlns="http://www.w3.org/2000/svg" className="h-2.5 w-2.5 md:h-3 md:w-3 inline-block text-blue-500" viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
              </svg>
              <svg xmlns="http://www.w3.org/2000/svg" className="h-2.5 w-2.5 md:h-3 md:w-3 inline-block text-blue-500 -ml-0.5 md:-ml-1" viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
              </svg>
            </>
          )}
